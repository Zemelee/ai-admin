<template>
  <div class="page-card">
    <div class="page-title">我带的学生</div>
    <el-table v-loading="loading" :data="rows" border stripe style="width: 100%">
      ...
      <el-table-column label="实习企业" min-width="200">
        <template #default="{ row }">
          {{ row.companyName || '—' }}
          <el-tag v-if="row.isBlacklistCompany" type="danger" size="small" style="margin-left: 4px">
            黑名单
          </el-tag>
        </template>
      </el-table-column>
...
onMounted(async () => {
  try {
    rows.value = await req('get', '/teacher/my-students')
  } finally {
    loading.value = false
  }
})
